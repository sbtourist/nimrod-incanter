...
(deftest test-alerts
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-alert :name "test" :metric "alert" :from 1 :to 2 :tag :no-tag)))]
...
(deftest test-alerts-with-tag
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-alert :name "test" :metric "alert" :from 1 :to 2 :tag "tag")))]
...
(deftest test-gauges
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-gauge :name "test" :metric "gauge" :from 1 :to 2 :tag :no-tag)))]
...
(deftest test-gauges-with-tag
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-gauge :name "test" :metric "gauge" :from 1 :to 2 :tag "tag")))]
...
(deftest test-counters
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-counter :name "test" :metric "counter" :from 1 :to 2 :tag :no-tag)))]
...
(deftest test-counters-with-tag
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-counter :name "test" :metric "counter" :from 1 :to 2 :tag "tag")))]
...
(deftest test-timers
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-timer :name "test" :metric "timer" :from 1 :to 2 :tag :no-tag)))]
    (is (= [:column-names ["timestamp" "timer"]] (first dataset)))
    (is (= [:rows '({"timer" 2, "timestamp" 2})] (second dataset)))))

(deftest test-timers-with-tag
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-timer :name "test" :metric "timer" :from 1 :to 2 :tag "tag")))]
