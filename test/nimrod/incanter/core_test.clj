...
(deftest test-alerts
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-alert "test" "alert" 1 :no-tag)))]
...
(deftest test-alerts-with-tag
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-alert "test" "alert" 1 "tag")))]
...
(deftest test-gauges
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-gauge "test" "gauge" 1 :no-tag)))]
...
(deftest test-gauges-with-tags
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-gauge "test" "gauge" 1 "tag")))]
...
(deftest test-counters
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-counter "test" "counter" 1 :no-tag)))]
...
(deftest test-counters-with-tags
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-counter "test" "counter" 1 "tag")))]
...
(deftest test-timers
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-timer "test" "timer" 1 :no-tag)))]
    (is (= [:column-names ["timestamp" "timer"]] (first dataset)))
    (is (= [:rows '({"timer" 2, "timestamp" 2})] (second dataset)))))

(deftest test-timers-with-tags
  (let [dataset (from-nimrod "http://localhost:8182" (from-log "1" (read-timer "test" "timer" 1 "tag")))]
